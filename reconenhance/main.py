...
from reconenhance.config import config, configurable_keys, configurable_boolean_keys
from reconenhance.io import slugify, e, fformat, cprint, debug, info, warn, error, fail, CommandStreamReader
from reconenhance.plugins import Pattern, PortScan, ServiceScan, Report, ReconEnhance
...
if not os.path.exists(config['config_dir']):
	...
	if not os.path.exists(os.path.join(config['config_dir'], 'VERSION-' + VERSION)):
		warn('It looks like the config/plugins in ' + config['config_dir'] + ' are outdated. Please remove the ' + config['config_dir'] + ' directory and re-run ReconEnhance to rebuild them.')

# Save current terminal settings so we can restore them.
config['terminal_settings'] = termios.tcgetattr(sys.stdin.fileno())

reconenhance = ReconEnhance()
...
def cancel_all_tasks(signal, frame):
	...
	for target in reconenhance.scanning_targets:
...
async def get_semaphore(reconenhance):
	semaphore = reconenhance.service_scan_semaphore
	while True:
		# If service scan semaphore is locked, see if we can use port scan semaphore.
		if semaphore.locked():
			if semaphore != reconenhance.port_scan_semaphore: # This will be true unless user sets max_scans == max_port_scans
				...
				if not reconenhance.pending_targets and (config['max_port_scans'] - port_scan_task_count) >= 1: # If no more targets, and we have room, use port scan semaphore.
					if reconenhance.port_scan_semaphore.locked():
						await asyncio.sleep(1)
						continue
					semaphore = reconenhance.port_scan_semaphore
					break
				else: # Do some math to see if we can use the port scan semaphore.
					if (config['max_port_scans'] - (port_scan_task_count + (len(reconenhance.pending_targets) * config['port_scan_plugin_count']))) >= 1:
						if reconenhance.port_scan_semaphore.locked():
							await asyncio.sleep(1)
							continue
						semaphore = reconenhance.port_scan_semaphore
...
async def port_scan(plugin, target):
	...
	async with target.reconenhance.port_scan_semaphore:
...
async def service_scan(plugin, service):
	semaphore = service.target.reconenhance.service_scan_semaphore

	if not config['force_services']:
		semaphore = await get_semaphore(service.target.reconenhance)
	...
	while plugin_pending:
		...
		if plugin.max_global_instances and plugin.max_global_instances > 0:
			async with service.target.reconenhance.lock:
				# Count currently running plugin instances.
				for target in service.target.reconenhance.scanning_targets:
		...
		async with semaphore:
			...
			nmap_extra = service.target.reconenhance.args.nmap
			if service.target.reconenhance.args.nmap_append:
				nmap_extra += ' ' + service.target.reconenhance.args.nmap_append
...
async def generate_report(plugin, targets):
	...
	if not config['force_services']:
		semaphore = await get_semaphore(reconenhance)
...
async def scan_target(target):
	...
	if config['force_services']:
		...
		if services:
			...
			heartbeat.cancel()
			reconenhance.errors = True
			return
	else:
		for plugin in target.reconenhance.plugin_types['port']:
			...
			if config['port_scans'] and plugin.slug in config['port_scans']:
				...
				matching_tags = False
				for tag_group in target.reconenhance.tags:
					if set(tag_group).issubset(plugin_tag_set):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in target.reconenhance.excluded_tags:
					if set(tag_group).issubset(plugin_tag_set):
						excluded_tags = True
						break
	...
	async with reconenhance.lock:
		reconenhance.scanning_targets.append(target)
	...
	while pending:
		...
		for service in services:
			...
			nmap_extra = target.reconenhance.args.nmap
			if target.reconenhance.args.nmap_append:
				nmap_extra += ' ' + target.reconenhance.args.nmap_append
			...
			for plugin in target.reconenhance.plugin_types['service']:
			...
			if not service_match:
				...
				if service.name not in ['status', 'tcpwrapped', 'unknown'] and service.full_tag() not in target.reconenhance.missing_services:
					target.reconenhance.missing_services.append(service.full_tag())

	for plugin in target.reconenhance.plugin_types['report']:
	...
	async with reconenhance.lock:
		reconenhance.completed_targets.append(target)
		reconenhance.scanning_targets.remove(target)
...
async def run():
	...
	parser.add_argument('-c', '--config', action='store', type=str, default=config_file, dest='config_file', help='Location of ReconEnhance\'s config file. Default: %(default)s')
	parser.add_argument('-g', '--global-file', action='store', type=str, help='Location of ReconEnhance\'s global file. Default: %(default)s')
	...
	parser.add_argument('--timeout', action='store', type=int, help='Specifies the maximum amount of time in minutes that ReconEnhance should run for. Default: %(default)s')
	...
	parser.add_argument('--proxychains', action='store_true', help='Use if you are running ReconEnhance via proxychains. Default: %(default)s')
	...
	parser.add_argument('--accessible', action='store_true', help='Attempts to make ReconEnhance output more accessible to screenreaders. Default: %(default)s')
	parser.add_argument('-v', '--verbose', action='count', help='Enable verbose output. Repeat for more verbosity.')
	parser.add_argument('--version', action='store_true', help='Prints the ReconEnhance version and exits.')
	parser.error = lambda s: fail(s[0].upper() + s[1:])
	args, unknown = parser.parse_known_args()

	errors = False

	reconenhance.argparse = parser

	if args.version:
		print('ReconEnhance v' + VERSION)
		sys.exit(0)
	...
	if not args.config_file:
		unknown_help()
		fail('Error: Could not find config.toml in the current directory or ~/.config/ReconEnhance.')
	...
	if not config['plugins_dir']:
		unknown_help()
		fail('Error: Could not find plugins directory in the current directory or ~/.config/ReconEnhance.')
			...
			if not plugin_file.startswith('_') and plugin_file.endswith('.py'):
				...
				try:
					...
					for (_, c) in clsmembers:
						if c.__module__ in ['reconenhance.plugins', 'reconenhance.targets']:
							continue
						...
						if issubclass(c, PortScan) or issubclass(c, ServiceScan) or issubclass(c, Report):
							reconenhance.register(c(), filename)
						else:
	...
	for plugin in reconenhance.plugins.values():
		if plugin.slug in reconenhance.taglist:
			unknown_help()
	...
	if len(reconenhance.plugin_types['port']) == 0:
		unknown_help()
		fail('Error: There are no valid PortScan plugins in the plugins directory "' + config['plugins_dir'] + '".')

	# Sort plugins by priority.
	reconenhance.plugin_types['port'].sort(key=lambda x: x.priority)
	reconenhance.plugin_types['service'].sort(key=lambda x: x.priority)
	reconenhance.plugin_types['report'].sort(key=lambda x: x.priority)

	if not config['global_file']:
		unknown_help()
		fail('Error: Could not find global.toml in the current directory or ~/.config/ReconEnhance.')
						...
						if 'pattern' in pattern:
							try:
								...
								if 'description' in pattern:
									reconenhance.patterns.append(Pattern(compiled, description=pattern['description']))
								else:
									reconenhance.patterns.append(Pattern(compiled))
							except re.error:
								unknown_help()
		...
		if key == 'global' and isinstance(val, dict): # Process global plugin options.
			for gkey, gval in config_toml['global'].items():
				if isinstance(gval, bool):
					for action in reconenhance.argparse._actions:
						if action.dest == 'global.' + slugify(gkey).replace('-', '_'):
							if action.const is True:
								action.__setattr__('default', gval)
							break
				else:
					if reconenhance.argparse.get_default('global.' + slugify(gkey).replace('-', '_')):
						reconenhance.argparse.set_defaults(**{'global.' + slugify(gkey).replace('-', '_'): gval})
		elif isinstance(val, dict): # Process potential plugin arguments.
			for pkey, pval in config_toml[key].items():
				if reconenhance.argparse.get_default(slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_')):
					reconenhance.argparse.set_defaults(**{slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_'): pval})
		else: # Process potential other options.
			key = key.replace('-', '_')
			if key in configurable_keys:
				other_options.append(key)
				config[key] = val
				reconenhance.argparse.set_defaults(**{key: val})
	...
	if config['max_plugin_target_instances']:
		for plugin_instance in config['max_plugin_target_instances']:
			...
			if len(plugin_instance) == 2:
				if plugin_instance[0] not in reconenhance.plugins:
	...
	if not errors:
		if config['force_services']:
			reconenhance.service_scan_semaphore = asyncio.Semaphore(config['max_scans'])
		else:
			reconenhance.port_scan_semaphore = asyncio.Semaphore(config['max_port_scans'])
			...
			if config['max_scans'] == config['max_port_scans']:
				reconenhance.service_scan_semaphore = reconenhance.port_scan_semaphore
			else:
				reconenhance.service_scan_semaphore = asyncio.Semaphore(config['max_scans'] - config['max_port_scans'])
	...
	[reconenhance.tags.append(t) for t in tags if t not in reconenhance.tags]
	...
	if args.exclude_tags != '':
		...
		# Remove duplicate lists from list.
		[reconenhance.excluded_tags.append(t) for t in excluded_tags if t not in reconenhance.excluded_tags]
	...
	for target in raw_targets:
		try:
			...
			found = False
			for t in reconenhance.pending_targets:
				if t.address == ip_str:
					found = True
					break
			...
			if isinstance(ip, ipaddress.IPv4Address):
				reconenhance.pending_targets.append(Target(ip_str, ip_str, 'IPv4', 'ip', reconenhance))
			elif isinstance(ip, ipaddress.IPv6Address):
				reconenhance.pending_targets.append(Target(ip_str, ip_str, 'IPv6', 'ip', reconenhance))
			else:
			...
			try:
				...
				try:
					...
					reconenhance.pending_targets.append(Target(target, ip, 'IPv4', 'hostname', reconenhance))
					...
					try:
						...
						reconenhance.pending_targets.append(Target(target, ip, 'IPv6', 'hostname', reconenhance))
					except socket.gaierror:
						error(target + ' does not appear to be a valid IP address, IP range, or resolvable hostname.')
						errors = True

	if len(reconenhance.pending_targets) == 0:
		error('You must specify at least one target to scan!')
		errors = True

	if config['single_target'] and len(reconenhance.pending_targets) != 1:
		error('You cannot provide more than one target when scanning in single-target mode.')
		errors = True

	if not args.disable_sanity_checks and len(reconenhance.pending_targets) > 256:
		error('A total of ' + str(len(reconenhance.pending_targets)) + ' targets would be scanned. If this is correct, re-run with the --disable-sanity-checks option to suppress this check.')
		errors = True

	if not config['force_services']:
		port_scan_plugin_count = 0
		for plugin in reconenhance.plugin_types['port']:
			if config['port_scans'] and plugin.slug in config['port_scans']:
				matching_tags = True
				excluded_tags = False
			else:
				matching_tags = False
				for tag_group in reconenhance.tags:
					if set(tag_group).issubset(set(plugin.tags)):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in reconenhance.excluded_tags:
					if set(tag_group).issubset(set(plugin.tags)):
						excluded_tags = True
						break
	...
	pending = []
	i = 0
	while reconenhance.pending_targets:
		pending.append(asyncio.create_task(scan_target(reconenhance.pending_targets.pop(0))))
		i+=1
	...
	while pending:
		...
		# If something failed in scan_target, reconenhance.errors will be true.
		if reconenhance.errors:
		...
		for task in done:
			if reconenhance.pending_targets:
				pending.add(asyncio.create_task(scan_target(reconenhance.pending_targets.pop(0))))
			if task in pending:
				pending.remove(task)

		port_scan_task_count = 0
		for targ in reconenhance.scanning_targets:
	...
	if len(reconenhance.completed_targets) > 1:
		for plugin in reconenhance.plugin_types['report']:
			plugin_tag_set = set(plugin.tags)
			...
			if matching_tags and not excluded_tags:
				pending.add(asyncio.create_task(generate_report(plugin, reconenhance.completed_targets)))
	...
	if timed_out:
		...
		elapsed_time = calculate_elapsed_time(start_time)
		warn('{byellow}ReconEnhance took longer than the specified timeout period (' + str(config['timeout']) + ' min). Cancelling all scans and exiting.{rst}')
	else:
	...
	if reconenhance.missing_services:
		warn('{byellow}ReconEnhance identified the following services, but could not match them to any plugins based on the service name.') 
