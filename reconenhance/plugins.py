...
from reconenhance.io import slugify, info, warn, error, fail, CommandStreamReader
from reconenhance.targets import Service
...
class Plugin(object):
	...
	def __init__(self):
		...
		self.priority = 1
		self.patterns = []
		self.reconenhance = None
		self.disabled = False

	@final
	def add_option(self, name, default=None, help=None):
		self.reconenhance.add_argument(self, name, metavar='VALUE', default=default, help=help)

	@final
	def add_constant_option(self, name, const, default=None, help=None):
		self.reconenhance.add_argument(self, name, action='store_const', const=const, default=default, help=help)

	@final
	def add_true_option(self, name, help=None):
		self.reconenhance.add_argument(self, name, action='store_true', help=help)

	@final
	def add_false_option(self, name, help=None):
		self.reconenhance.add_argument(self, name, action='store_false', help=help)

	@final
	def add_list_option(self, name, default=None, help=None):
		self.reconenhance.add_argument(self, name, nargs='+', metavar='VALUE', default=default, help=help)
	...
	def add_choice_option(self, name, choices, default=None, help=None):
		...
		self.reconenhance.add_argument(self, name, choices=choices, default=default, help=help)

	@final
	def get_option(self, name):
		name = self.slug.replace('-', '_') + '.' + slugify(name).replace('-', '_')

		if name in vars(self.reconenhance.args):
			return vars(self.reconenhance.args)[name]
		else:
			return None
	...
	def get_global_option(self, name, default=None):
		...
		if name in vars(self.reconenhance.args):
			if vars(self.reconenhance.args)[name] is None:
				if default:
					return default
				else:
					return None
...
class ReconEnhance(object):
	...
	async def execute(self, cmd, target, tag, patterns=None, outfile=None, errfile=None):
		...
		print()
		warn('{byellow}ReconEnhance wants to execute the following command:{rst} {cmd}')
		#warn('{byellow}ReconEnhance wants to execute this command:{rst} {cmd}')
		while True:
